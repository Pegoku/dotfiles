...
export default () => {
    ...
    const SystemResourcesOrCustomModule = () => {
        ...
        if (GLib.file_test(CUSTOM_MODULE_CONTENT_SCRIPT, GLib.FileTest.EXISTS)) {
            ...
            child: Box({
                children: [
                    BarResource('RAM Usage', 'Album', `LANG=C free | awk '/^Mem/ {printf("%.2f\\n", ($3/$2) * 100)}'`,
                    ...
                    BarResource('GPU Usage', 'Settings_Input_Hdmi', `nvidia-smi --query-gpu=utilization.gpu --format=csv,noheader,nounits`),
                ],
            })
        });
    }
