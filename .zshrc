...
## fnm
#export PATH="/home/pegoku/.local/share/fnm:$PATH"
#eval "`fnm env`"
...
_lazy_load_dart() {
  ...
  add-zsh-hook -d chpwd _lazy_load_dart
}
add-zsh-hook chpwd _lazy_load_dart
## [/Completion]
...
unalias ga gc gn gp 2>/dev/null
...
gc() {
  git commit --message "${1:-commit}"
}

gn() {
  git add "${1:-.}"
  git commit --message "${2:-commit}"
}
